...
void PersistMapRingBuffer::processMesh(const ColorMesh& mesh)
{
    ...
    //if (current->needDraw(g_cofficient))
    //{
    //    drawCurrentSegmentBuffer();
    //}
}
...
PersistMapStipple::PersistMapStipple()
{
}
...
void PersistMapStipple::initBuffer()
{
    constexpr int ver_size = sizeof(ColorStipVertex);
    std::size_t kTotalVBOSize = MemorySegment::SegmentVertexCapacity() * kSEG_NUM * sizeof(ColorStipVertex);
    std::size_t kTotalEBOSize = MemorySegment::SegmentIndiceCapacity() * kSEG_NUM * sizeof(unsigned int);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, m_vbo);
    glBufferStorage(GL_ARRAY_BUFFER, kTotalVBOSize, nullptr, createFlags);
    m_perMapVertices = reinterpret_cast<ColorStipVertex*> (glMapBufferRange(GL_ARRAY_BUFFER, 0, kTotalVBOSize, mapFlags));

    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(ColorStipVertex), (void*)0);
    glEnableVertexAttribArray(0);
...
void PersistMapStipple::bindVAO()
{
    glBindVertexArray(m_vao);
}

void PersistMapStipple::processMesh(const ColorStipMesh& mesh)
{
    const std::vector<ColorStipVertex>& vertices = mesh.getVertices();
    const std::vector<unsigned int>& indices = mesh.getIndices();
    std::size_t vertice_count = vertices.size();
    std::size_t indice_count = indices.size();
    MemorySegment* current = &(m_segments[m_cur_seg_idx]);
    ...
    // 1 copy batch vertices
    std::size_t global_vertice_offset = current->globalVertexOffset();
    ColorStipVertex* dest = m_perMapVertices + global_vertice_offset;
    const ColorStipVertex* src = vertices.data();
    std::memcpy(dest, src, vertices.size() * sizeof(ColorStipVertex));
    // 2 copy batch indices with offset;
    std::size_t global_indice_offset = current->globalIndexOffset();
    unsigned int* idx_dest = m_perMapIndices + global_indice_offset;
    unsigned int offset = unsigned int(global_vertice_offset);
...
void PersistMapStipple::setCurrentDrawMode(DrawMode mode)
{
    m_draw_mode = mode;
}

void PersistMapStipple::drawCurrentSegmentBuffer()
{
    MemorySegment* current = &(m_segments[m_cur_seg_idx]);
    GLsizei count = int(current->drawElementCount());
    if (0 == count)
